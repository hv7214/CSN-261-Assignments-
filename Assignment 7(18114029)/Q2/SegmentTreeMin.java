...
public class SegmentTreeMin {
    ...
    private int buildTreeUtil(int[] arr, int l, int r, int pos) {
            if (l == r) {
                tree[pos] = (l < arr_size ? arr[l] : 100000000);
                return tree[pos];
            }

            int mid = (l + r) / 2;
            tree[pos] = Math.min(buildTreeUtil(arr, l, mid, 2*pos+1), buildTreeUtil(arr, mid+1, r, 2*pos+2));
    ...
    private int getMinUtil(int lptr, int rptr, int pos, int l, int r) {
        ...
        int mid = (lptr + rptr) / 2;
        return Math.min(getMinUtil(lptr, mid, 2*pos+1, l, r), getMinUtil(mid+1, rptr, 2*pos+2, l, r));
    }

    public int getMin(int l, int r) {
        if (l < 0  || r > this.arr_size) 
            return -1;
        return getMinUtil(this.sindex, this.eindex, 0, l, r);    
    }
